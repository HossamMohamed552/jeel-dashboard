  ...
  <section class="inner-routes">
    <div class="taps">
      <div @click="activeTapActive(1)"
           ...
           class="tap" v-if="Array.from(routesUsers).length>= 1">صلاحيات النظام والمستخدمين
      </div>
      <div @click="activeTapActive(2)"
           :class="activeTap === 2 && Array.from(routesSchool).length>= 1 ? 'active' : ''"
           class="tap" v-if="Array.from(routesSchool).length>= 1">المدارس
      </div>
      <div @click="activeTapActive(3)"
           :class="activeTap === 3 && Array.from(routesContent).length>= 1? 'active' : ''"
           class="tap" v-if="Array.from(routesContent).length>= 1">المحتوي
      </div>
    </div>
    <div class="content">
      <div class="row">
